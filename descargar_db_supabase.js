...
async function obtenerEsquemaSQL() {
  try {
    ...
    // Leer el esquema existente si está disponible
    const schemaPath = path.join(__dirname, 'emop_app', 'supabase_schema.sql');
    if (fs.existsSync(schemaPath)) {
      console.log('  ✅ Usando esquema existente de supabase_schema.sql');
      return fs.readFileSync(schemaPath, 'utf8');
    }
    
    // Si no existe, crear un esquema básico
    return '-- Esquema de base de datos EMOP\n-- Generado automáticamente\n';
  } catch (err) {
...
async function main() {
  ...
  const outputDir = path.join(__dirname, 'backup_supabase');
