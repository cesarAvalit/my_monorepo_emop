...
-- =====================================================
-- Fecha de backup: 2026-01-08T20:58:21.072Z
-- Base de datos: https://weberwavolitwvmjfhap.supabase.co
-- 
...
CREATE TABLE IF NOT EXISTS inspeccion_ddjj (
    ...
    id_usuario INTEGER REFERENCES usuario(id_usuario),
    id_orden_trabajo INTEGER REFERENCES orden_trabajo(id_orden),
    licencia_correcto BOOLEAN,
    rto_correcto BOOLEAN,
    seguro_correcto BOOLEAN,
    aire_acondicionado_correcto BOOLEAN,
    calefaccion_correcto BOOLEAN,
    camara_correcto BOOLEAN,
    observaciones TEXT,
    fecha_creacion TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS notificaciones (
    id SERIAL PRIMARY KEY,
    id_tipo_notificacion INTEGER REFERENCES tipo_notificacion(id),
    id_usuario INTEGER REFERENCES usuario(id_usuario),
    fecha_hora TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    nota TEXT,
...
-- =====================================================
-- TABLA: ddjj_x_usuario (Pivot)
-- =====================================================
CREATE TABLE IF NOT EXISTS ddjj_x_usuario (
    id SERIAL PRIMARY KEY,
    id_ddjj INTEGER REFERENCES declaracion_jurada(id_ddjj),
    id_usuario INTEGER REFERENCES usuario(id_usuario),
...
CREATE TRIGGER update_reporte_auditoria_ddjj_updated_at BEFORE UPDATE ON reporte_auditoria_ddjj
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_inspeccion_ddjj_updated_at BEFORE UPDATE ON inspeccion_ddjj
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_tipo_notificacion_updated_at BEFORE UPDATE ON tipo_notificacion
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_notificaciones_updated_at BEFORE UPDATE ON notificaciones
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_declaracion_jurada_updated_at BEFORE UPDATE ON declaracion_jurada
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
